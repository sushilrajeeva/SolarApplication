...
if (signupForm) {
  signupForm.addEventListener('submit', function (event) {
    console.log("SignupForm js file is triggered!");
    ...
    let retypePasswordInput = document.getElementById('retypePasswordInput');
    ...
    address = checkEmptyInputString(address, "Address");
    ...
    checkNameInput(firstName);
    checkNameInput(middleName);
    checkNameInput(lastName);
    ...
    checkPasswordsMatch(password, retypePassword);
    ...
    retypePasswordInput.value = retypePassword;
...
function checkNameInput(name, type){
    let nameRegex = /^[A-Za-z]{1,25}$/.test(name);

    if(!nameRegex){
        throw `Error: ${name} should be a valid ${type}!, should be 1 to 25 charecters long, non empty spaces containing only alphabets and no numbers..`
    }
...
function checkValiduserType(userType){
    let validUserTypes = ["normal user", "sales", "management", "crew"];
